...
int	init_user_inputs(t_data *data, char **av)
{
	...
	if (data->time_to_die <= 0)
		return (init_data_print_error("Incorrect time_to_die\n",
				INVALID_ARG));
	data->time_to_eat = ft_atoi(av[3]);
	if (data->time_to_eat <= 0)
		return (init_data_print_error("Incorrect time_to_eat\n",
				INVALID_ARG));
	data->time_to_sleep = ft_atoi(av[4]);
	if (data->time_to_sleep <= 0)
		return (init_data_print_error("Incorrect time_to_sleep\n",
				INVALID_ARG));
	if (av[5])
